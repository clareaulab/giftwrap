...
def make_pdf_report(output_file, gapfill_adata, adata):
    ...
    with PdfPages(output_file) as pdf:
        ...
        axs[0].axvline(x=gapfill_adata.shape[1], color='r', linestyle='--')
        ...
        gapfills_per_gene = np.zeros((gapfill_adata.shape[0],len(unique_genes)), dtype=int)
        for i, gene in enumerate(unique_genes):
            gapfills_per_gene[:,i] = gapfill_adata[:, gapfill_adata.var['probe'] == gene].X.sum(axis=1).flatten()
        axs[0].boxplot(gapfills_per_gene, labels=unique_genes)
        axs[0].set_ylabel("UMIs")
        axs[0].set_xlabel("Probe")

        axs[1].hist((gapfills_per_gene > 0).sum(axis=0), bins=100)
        axs[1].axvline(x=len(unique_genes), color='r', linestyle='--')
        axs[1].set_xlabel("Genes containing gapfill")
        axs[1].set_ylabel("Frequency")
        ...
        ax.boxplot(gapfill_adata.layers['total_reads'], labels=gapfill_adata.var['probe'].tolist())
        ax.set_yscale("log")
        ax.set_ylabel("Reads")
        ax.set_xlabel("Probe")
        fig.text(0.5, 0.05,
                 "This figure shows the distributions of reads supporting each gapfill. "
        ...
        try:
            ...
            return
        # Save the raw data
        gapfill_adata.raw = gapfill_adata
        # Normalize the data
        ...
        sc.pl.tsne(gapfill_adata, color='cluster', ax=axs[0])
        sc.pl.umap(gapfill_adata, color='cluster', ax=axs[1])
        fig.text(0.5, 0.05,
        ...
        if adata is not None:
            ...
            gapfill_adata = gapfill_adata.raw
            # Re-order cells in adata to match gapfill adata
            adata = adata[gapfill_adata.obs.index, :]
            # Get the gene names
            if 'gene' not in adata.var.columns:  # No gene name included in manifest, we will try to guess the gene name
            ...
            axs[0].plot(gapfill_expression.flatten(), best_fit(gapfill_expression.flatten(), wta_expression.flatten()),
                        zorder=1, linestyle='--')
            ...
            # Plot a density plot of the correlation
            axs[1].scatter(gapfill_expression, wta_expression, c=density(gapfill_expression, wta_expression))
            # Add a line of best fit
            axs[0].plot(gapfill_expression.flatten(), best_fit(gapfill_expression.flatten(), wta_expression.flatten()),
                        zorder=1, linestyle='--')
            ...
            fig.text(0.5, 0.05, "This figure shows the correlation between the expression of genes in the gapfill and WTA data. "
                                ...
                                "The density of points is shown in the colors and a line of best shown with dashes. "
            ...
            gapfill_adata.obsm['X_umap'] = adata.obsm['X_umap'][gapfill_adata.obs.index]

            # Plot summary statistics
            fig, axs = plt.subplots(1, 3, figsize=(8, 8))
            fig.set_dpi(600)
            fig.suptitle("Gapfill on WTA UMAP")
            sc.pl.umap(gapfill_adata, color='umis_per_cell', ax=axs[0], title="UMIs per cell")
            ...
            for i, probe in enumerate(gapfill_adata.var['probe'].unique()):
                probe_X[i] = gapfill_adata[:, gapfill_adata.var['probe'] == probe].X.sum()
            ...
            for i, probe in enumerate([max_probe, min_probe]):
                ...
                sc.pl.umap(gapfill_adata, color='majority_gapfill', ax=ax, title=f"Majority gapfill for {probe}")

            fig.text(0.5, 0.05,
                    "The top figure shows the distribution of UMIs per cell on the WTA UMAP. "
            "The middle and bottom figures show the majority gapfill for the probe with the most and fewest UMIs, respectively.",
                    ha='center', wrap=True)
            pdf.savefig(fig)
            plt.close(fig)
...
def summarize_counts(input: Path, summary_output: Path, summary_pdf_output: Path, counts_output: Path, cellranger_output: Optional[Path]):
    print("Summarizing counts file ", input, " to ", summary_output, ", ", summary_pdf_output, ", and ", counts_output, "...", end="")
    ...
    if adata:
        ...
        print(f"Filtered to {gapfill_adata.shape[1]} gapfill cells.")
    else:
    ...
    stats["value"].append(gapfill_adata.shape[1])
...
def run(output, overwrite, cellranger_output):
    ...
    for i, counts_file in enumerate(counts_files):
        ...
        summary_output_file = output / counts_file.name.replace(".h5", ".summary.csv")
